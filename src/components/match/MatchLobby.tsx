...
import { ArrowLeft, Swords, BookOpen, Trophy, Dice5, AlertTriangle, RefreshCw } from 'lucide-react'
...
export function MatchLobby({ currentUser, onExit }: Props) {
  ...
  const handleChallenge = async (friend: Friend) => {
    ...
    try {
      ...
      // Tenta parsear JSON mesmo em status de erro
      let data: any
      try {
        data = await res.json()
      } catch {
        // Se não consegue parsear JSON, pode ser erro do Vercel/serverless
        if (res.status === 401) {
      ...
      if (!data.ok) {
        ...
        // Se o erro menciona banco de dados, mostra aviso visual
        if (errorMsg.includes('banco') || errorMsg.includes('SQL') || errorMsg.includes('coluna') || errorMsg.includes('tabela')) {
          setDbWarning(errorMsg)
        }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-emerald-950/30 to-gray-950 text-white">
      ...
      <main className="mx-auto max-w-5xl px-4 py-6">
        ...
        {/* Aviso de problema no banco de dados */}
        {dbWarning && (
          <motion.div
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold text-amber-300">Problema no banco de dados</p>
                <p className="mt-1 text-xs text-amber-200/80">{dbWarning}</p>
                <p className="mt-2 text-xs text-gray-400">
                  Execute o arquivo <code className="rounded bg-gray-800 px-1 text-amber-300">sql-setup-complete.sql</code> no
                  Neon Console para corrigir.
                </p>
              </div>
              <Button
                variant="ghost"
