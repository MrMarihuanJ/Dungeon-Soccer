...
// FREE_KICK e KICKOFF são excluídas: KICKOFF só no 1º turno, FREE_KICK só na cobrança de falta
export function sampleMixedActions(count: number): FootballAction[] {
  const pool = ALL_ACTIONS.filter((a) => a.category !== 'KICKOFF' && a.category !== 'FREE_KICK')
  const shuffled = [...pool].sort(() => Math.random() - 0.5)
  return shuffled.slice(0, Math.min(count, pool.length))
}
